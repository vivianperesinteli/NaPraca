 ...
 * Esta função usa a RPC do Supabase que verifica antes de criar.
 */
export async function ensureMyProfileId(): Promise<string | null> {
  ...
  try {
    ...
    if (error) {
      // Se o erro é 409 (conflict) ou perfil já existe, tentar buscar o perfil diretamente
      if (error.code === 'PGRST116' || error.message?.includes('409') || error.message?.includes('duplicate')) {
    ...
    console.warn("Erro ao garantir perfil:", error);
    // Última tentativa: buscar o perfil diretamente
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        const { data: profile } = await supabase
